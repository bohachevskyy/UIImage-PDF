...
{
    return [ self imageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName ] atSize:size atPage:page ];
}

+(UIImage *) imageWithPDFNamed:(NSString *)resourceName atSize:(CGSize)size
{
    return [ self imageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName ] atSize:size ];
}

+(UIImage *) imageWithPDFNamed:(NSString *)resourceName atWidth:(CGFloat)width atPage:(NSUInteger)page
{
    return [ self imageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName ] atWidth:width atPage:page ];
}

+(UIImage *) imageWithPDFNamed:(NSString *)resourceName atWidth:(CGFloat)width
{
    return [ self imageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName ] atWidth:width ];
}

+(UIImage *) imageWithPDFNamed:(NSString *)resourceName atHeight:(CGFloat)height atPage:(NSUInteger)page
{
    return [ self imageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName ] atHeight:height atPage:page ];
}

+(UIImage *) imageWithPDFNamed:(NSString *)resourceName atHeight:(CGFloat)height
{
    return [ self imageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName ] atHeight:height ];
}

+(UIImage *) imageWithPDFNamed:(NSString *)resourceName fitSize:(CGSize)size atPage:(NSUInteger)page
{
    return [ self imageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName] fitSize:size atPage:page ];
}

+(UIImage *) imageWithPDFNamed:(NSString *)resourceName fitSize:(CGSize)size
{
    return [ self imageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName] fitSize:size ];
}

+(UIImage *) originalSizeImageWithPDFNamed:(NSString *)resourceName atPage:(NSUInteger)page
{
    return [ self originalSizeImageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName ] atPage:page ];
}

+(UIImage *) originalSizeImageWithPDFNamed:(NSString *)resourceName
{
    return [ self originalSizeImageWithPDFURL:[ SkychutePDFView resourceURLForName:resourceName ]];
}
...
{
    CGRect mediaRect = [ SkychutePDFView mediaRectForData:data atPage:1 ];
...
+(UIImage *)imageWithPDFData:(NSData *)data atWidth:(CGFloat)width atPage:(NSUInteger)page
{
    ...
    CGRect mediaRect = [ SkychutePDFView mediaRectForData:data atPage:page ];
...
+(UIImage *)imageWithPDFData:(NSData *)data atSize:(CGSize)size atPage:(NSUInteger)page preserveAspectRatio:(BOOL)preserveAspectRatio
{
    ...
    else
    {
        ...
        [SkychutePDFView renderIntoContext:ctx url:nil data:data size:size page:page preserveAspectRatio:preserveAspectRatio];
...
+(UIImage *) imageWithPDFURL:(NSURL *)URL atSize:(CGSize)size atPage:(NSUInteger)page preserveAspectRatio:(BOOL)preserveAspectRatio
{
    ...
    else 
    {
        ...
        [SkychutePDFView renderIntoContext:ctx url:URL data:nil size:size page:page preserveAspectRatio:preserveAspectRatio];
        CGImageRef image = CGBitmapContextCreateImage(ctx);
        pdfImage = [[UIImage alloc] initWithCGImage:image scale:screenScale orientation:UIImageOrientationUp];
...
{
    CGRect mediaRect = [ SkychutePDFView mediaRectForURL:URL atPage:page ];
    CGFloat aspectRatio = mediaRect.size.width / mediaRect.size.height;
